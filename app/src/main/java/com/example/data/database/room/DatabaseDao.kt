...
interface DatabaseCoinsDao{
    ...
    fun getPriceList(): Flow<List<DbModelCoinPriceInfo>>                       //Вся информация о всех сохраненных данных о валютах в локальной базе

    @Query("SELECT * FROM full_price_list WHERE fromSymbol == :fSym")
    fun getPriceInfoAboutCoin(fSym:String): Flow<DbModelCoinPriceInfo>        //Вся детальная информация по одной валюте
